...
//                $mstore->setMoneyFormat($store['money_format']);
	...
	public function getJsUrl($storeId)
	{
		...
		if ($this->getConfigValue(self::XML_PATH_ACTIVE, $storeId) && !$url) {
			 ...
			 * `Ebizmarts_MailChimp`:
			 * «API Key Invalid for Api Call: https://us6.api.mailchimp.com/3.0/ecommerce/stores/-1 -
			 * Your API key may be invalid, or you've attempted to access the wrong datacenter»
			 * https://github.com/tradefurniturecompany/core/issues/22
			 */
			if (-1 < (int)$mailChimpStoreId) {
