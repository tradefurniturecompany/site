 ...
 *
 */
class Customweb_Realex_BackendOperation_Adapter_RefundXmlBuilder extends Customweb_Realex_BackendOperation_AbstractXmlBuilder {
	private $amount = null;
	...
	public function buildXml() {
		return  $this->getXMLHeader('rebate') .
		$this->getBasicElements() .
		$this->getOrderIdElement() .
		$this->getPasrefElement() .
		$this->getAuthcodeElement() .
		$this->getAmountElement($this->amount) .
		$this->getRefundHashElement() .
		$this->getHashElement() . 
		$this->getXMLFooter();
	}
	
	protected function getRefundHashElement() {
		$refundHash = sha1($this->getConfiguration()->getRefundPassword());
		return '<refundhash>' . $refundHash . '</refundhash>';
	}
